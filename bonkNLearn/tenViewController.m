...
-(void)motionBegan:(UIEventSubtype)motion withEvent:(UIEvent *)event {
    ...
    //
    NSString *music = [[NSBundle mainBundle]pathForResource:@"10" ofType:@"mp3"];
...
-(void)delay
{
    [self performSelector:@selector(tenPlayed)
               withObject:(nil)
               afterDelay:(1.0)];
...
- (void)menuPlay
{
    [self performSegueWithIdentifier:@"tenToMenu" sender:nil];
