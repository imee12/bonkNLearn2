...
-(void)motionBegan:(UIEventSubtype)motion withEvent:(UIEvent *)event {
    ...
    //
    NSString *music = [[NSBundle mainBundle]pathForResource:@"7" ofType:@"mp3"];
...
-(void)delay
{
    [self performSelector:@selector(sevenPlayed)
               withObject:(nil)
               afterDelay:(1.0)];
...
- (void)eightPlay
{
    [self performSegueWithIdentifier:@"sevenToEight" sender:nil];
