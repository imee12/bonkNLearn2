...
-(void)motionBegan:(UIEventSubtype)motion withEvent:(UIEvent *)event {
    ...
    //
    NSString *music = [[NSBundle mainBundle]pathForResource:@"5" ofType:@"mp3"];
...
-(void)delay
{
    [self performSelector:@selector(fivePlayed)
               withObject:(nil)
               afterDelay:(1.0)];
...
- (void)sixPlay
{
    [self performSegueWithIdentifier:@"fiveToSix" sender:nil];
