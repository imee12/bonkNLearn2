...
-(void)motionBegan:(UIEventSubtype)motion withEvent:(UIEvent *)event {
    ...
    //
    NSString *music = [[NSBundle mainBundle]pathForResource:@"Nueve" ofType:@"mp3"];
...
-(void)delay
{
    [self performSelector:@selector(nuevoPlayed)
               withObject:(nil)
               afterDelay:(1.0)];
...
- (void)diezPlay
{
    [self performSegueWithIdentifier:@"nuevoToDiez" sender:nil];
